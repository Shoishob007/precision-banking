    ...
    <ProtectedRoute>
      ...
      <aside className="hidden lg:flex flex-col w-64 fixed left-0 top-0 h-full bg-surface-container-low border-r border-outline-variant/10 z-50">
        <div className="px-6 py-8">
          <Link href="/dashboard" className="text-xl font-black text-on-surface tracking-tighter">Precision Editorial</Link>
        </div>
        ...
        <div className="p-4 mt-auto border-t border-outline-variant/10">
          {bottomNavItems.map((item) => (
            <Link
              key={item.id}
              href={item.path}
              className="flex items-center w-full gap-3 px-4 py-3 text-on-surface-variant font-sans text-[10px] font-bold uppercase tracking-widest hover:text-on-surface hover:bg-surface-container-high transition-all rounded-lg"
            >
              <item.icon size={18} />
              <span>{item.label}</span>
            </Link>
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-surface/80 backdrop-blur-xl border-t border-outline-variant/10 z-50">
          <div className="flex justify-around items-center px-2 py-3">
            ...
            <button className="flex flex-col items-center gap-1 text-on-surface-variant">
              <Wifi size={20} />
              <span className="text-[10px] font-medium uppercase">Alerts</span>
            </button>
            <button 
              onClick={logout}
